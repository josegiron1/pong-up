ALTER TABLE `users_to_companies` RENAME COLUMN `group_id` TO `company_id`;--> statement-breakpoint
/*
You're trying to delete PRIMARY KEY(group_id,user_id) from 'users_to_companies' table
...
/*
 ...
 Due to that we don't generate migration automatically and it has to be done manually
*/--> statement-breakpoint
DROP INDEX IF EXISTS `company_id_idx`;--> statement-breakpoint
CREATE INDEX `company_id_idx` ON `users_to_companies` (`company_id`);--> statement-breakpoint
/*
...
*/--> statement-breakpoint
/*
You're trying to add PRIMARY KEY(company_id,user_id) to 'users_to_companies' table
